...
for ln_pair in "roen" "deen" "enfr"; do
  ...
  ../scripts/create_fast_align_corpus.sh ../test/${ln_pair}.lc.src ../test/${ln_pair}.lc.tgt ${ln_pair}.test.fastalign
  # running forced align by hand for each direction requires parsing the error file of the training pass
  ${FASTALIGN_DIR}/build/force_align.py ${ln_pair}.model ${ln_pair}.error ${ln_pair}.reverse.model ${ln_pair}.reverse.error < ${ln_pair}.test.fastalign > roen.hyp.merge.talp
done
